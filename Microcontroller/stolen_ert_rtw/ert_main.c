...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) stolen_M->Timing.clockTick0;
  stolen_step();
...
int main(void)
{
  ...
  errorCode = extmodeInit(stolen_M->extModeInfo, &rtmGetTFinal(stolen_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(stolen_M, true);
    }
  }
