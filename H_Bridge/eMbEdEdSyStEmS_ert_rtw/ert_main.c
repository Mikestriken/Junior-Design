...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(eMbEdEdSyStEmS_M, 0);

  /* Set Final Simulation Time in Ticks */
  errorCode = extmodeSetFinalSimulationTime((extmodeSimulationTime_T) 1000000);
  ...
  eMbEdEdSyStEmS_initialize();
  globalInterruptDisable();
  globalInterruptEnable();

  /* External Mode initialization */
  errorCode = extmodeInit(eMbEdEdSyStEmS_M->extModeInfo, &rtmGetTFinal
    (eMbEdEdSyStEmS_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(eMbEdEdSyStEmS_M, true);
    }
  }
